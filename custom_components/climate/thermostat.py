...
async def async_setup_platform(hass, config, async_add_entities,
    ...
    async_add_entities([GenericThermostat(
        hass, name, heater_entity_id, cooler_entity_id, sensor_entity_id, min_temp, max_temp,
...
class GenericThermostat(ClimateDevice):
    ...
    def __init__(self, hass, name, heater_entity_id, cooler_entity_id, sensor_entity_id,
        ...
        if self.ac_mode:
            ...
            self._operation_list = [STATE_COOL, STATE_HEAT, STATE_OFF]
        else:
            self._current_operation = STATE_HEAT
            self._operation_list = [STATE_HEAT, STATE_COOL, STATE_OFF]
        ...
        async_track_state_change(
            hass, cooler_entity_id, self._async_switch_changed)
        ...
        _LOGGER.debug("state = %s", sensor_state)
    ...
    async def _async_sensor_changed(self, entity_id, old_state, new_state):
        ...
        _LOGGER.debug("old state from sensor: %s", old_state.state)
        _LOGGER.debug("new state from sensor: %s", new_state.state)
    ...
    def _async_update_temp(self, state):
        ...
        try:
            ...
            _LOGGER.debug("update from sensor: %s", state.state)
    ...
    async def _async_control_heating(self, time=None):
        ...
        async with self._temp_lock:
            if not self._active and None not in (self._cur_temp,
                ...
                _LOGGER.info("Didnt Obtain current and target temperature. "
                             "Generic thermostat not active. %s, %s",
                             self._cur_temp, self._target_temp)
